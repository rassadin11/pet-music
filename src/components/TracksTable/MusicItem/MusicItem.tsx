...
import {getTrack} from '../../../store/chart/chart.thunks'

const MusicItem = memo(({item, idx, without}: MusicItemProps) => {
  const dispatch = useDispatch<AppDispatch>()
  // проверяем из сколько столбцов состоит таблица
  const [isThreeColumns] = useState<boolean>(
    without?.length ? true : false,
  )
  ...
  return (
    <tr className={s.music}>
      ...
      {Number(without?.findIndex(s => s == 'artistName')) > -1 ? (
        ...
        <td
          className={cn(
            isThreeColumns ? s.artistName : s.bigArtistName,
          )}
        >
          <Link to={`/musicians/${item.artist.name}`}>
            {item.artist.name}
          </Link>
        </td>
      )}
      ...
      {Number(without?.findIndex(s => s == 'listens')) > -1 ? (
        ...
        <td
          className={cn(
            isThreeColumns ? s.bigPlaycount : s.playcount,
          )}
        >
