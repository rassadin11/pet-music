...
import {
  getMusicianInfo,
  getTracksByMusician,
} from '../../store/musicians/musicians.thunks'
...
const MusicianPage = () => {
  ...
  useEffect(() => {
    if (
      !authorName ||
      (activeMusician?.name.toLowerCase() ===
  ...
  useEffect(() => {
    setListens(
      validateListeners(activeMusician?.stats?.listeners || 0),
    )
  ...
  return (
    <section className={s.musician}>
      ...
      <section>
        ...
        <AlbumsBlock
          albums={activeMusician.albums?.map(modifyAlbums)}
        />
      </section>
